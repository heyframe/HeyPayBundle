...
namespace HeyPay\Bundle\PayBundle\Profiler;
...
class PayCollector extends DataCollector implements ExtensionInterface
{
    /**
     * @var Context[]
     */
    ...
    public function dump(): string
    {
        ...
        foreach ($this->data as $index => $contextData) {
            ...
            if (\array_key_exists($index + 1, $this->data) === false && $contextData['reply_class']) {
                $str .= $this->formatReply($contextData) . \PHP_EOL;
            }

            if (\array_key_exists($index + 1, $this->data) === false && $contextData['exception_class']) {
